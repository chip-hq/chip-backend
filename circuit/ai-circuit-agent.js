...
Be clear and concise. Do not add unsolicited recommendations or repeat the canvas output names.`;
...
async function compileAutomation(definition, userId, board = 'esp32') {
  ...
  createJob({
    ...
    status: 'compiling',
    progress: 0,
    log: ['Automation compile started…', `Components: ${components.join(', ') || 'none'}`],
  });
  try {
    const result = await compileFirmware({
      ...
      board,
      components,
      libraries: librariesForComponents(components),
      jobId,
      onLog: (line) => updateJob(jobId, { logLine: line }),
    });
    updateJob(jobId, {
      status: 'done',
      progress: 100,
